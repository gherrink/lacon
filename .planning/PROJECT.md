...
**None — all v1 requirements are shipped across Phases 1–8** (see Validated above; full REQ-ID list in `.planning/REQUIREMENTS.md`). The v1.0 milestone is code-complete: 8 engine + 5 adapter + 5 tracking + 7 CLI + 2 bundled-rule + 6 acceptance + 3 documentation requirements all closed, plus two post-audit polish phases (7: raw-output capture; 8: stats read-time presentation per ADR 0014). The only open follow-up is confirming the `macos-latest` CI lane on first push (tracked in `.planning/phases/06-v1-ship-gate-acceptance-docs/06-HUMAN-UAT.md`). Post-v1 candidates live in `docs/backlog.md`.
...
---
*Last updated: 2026-05-23 after Phase 8 (redesign `lacon stats` output for readability — ADR 0014) completed — verification 7/7 must-haves, code review 2 blockers / 3 warnings all fixed. All eight phases of the v1.0 milestone are now complete: the full engine, tracking, Claude Code adapter, six-command CLI, ten bundled rules, hermetic CI, README/worked-example/primitive-reference docs, raw-output capture for `lacon explain` (Phase 7), and the `lacon stats` read-time presentation layer (Phase 8).*
