...
**None — all v1 requirements are shipped across Phases 1–6** (see Validated above; full REQ-ID list in `.planning/REQUIREMENTS.md`). The v1.0 milestone is code-complete: 8 engine + 5 adapter + 5 tracking + 7 CLI + 2 bundled-rule + 6 acceptance + 3 documentation requirements all closed. The only open follow-up is confirming the `macos-latest` CI lane on first push (tracked in `.planning/phases/06-v1-ship-gate-acceptance-docs/06-HUMAN-UAT.md`). Post-v1 candidates live in `docs/backlog.md`.
...
**Project status (2026-05-22):** v1.0 milestone code-complete — all 6 phases done (engine + `lacon run`, local tracking, Claude Code adapter + `lacon init`, CLI completion, bundled Tier 1 rules, ship gate + docs). The Rust workspace ships `crates/lacon-core`, `crates/lacon-cli`, `crates/lacon-adapter-claudecode`, `bin/test_emitter`, `bundled-rules/`, and `tests/fixtures/`; `cargo test --workspace` is green (448 passed). The 13 ADRs in `docs/decisions/`, 4 specs in `docs/specs/`, and 2 PRDs (`docs/v1-scope.md`, `docs/vision.md`) remain the authoritative contract. Treat them as the source of truth — proposed changes that contradict an ADR must surface that explicitly. Next: confirm the `macos-latest` CI lane on first push (the one tracked human follow-up), then the v1.0 milestone is ready to audit/complete.
...
---
*Last updated: 2026-05-22 after Phase 6 (v1 ship gate — acceptance & docs) completed — verification 9/9 must-haves (status human_needed: macOS CI lane awaiting first run, accepted by user as a tracked follow-up), code review 0 blockers / 6 warnings all fixed. All six phases of the v1.0 milestone are now complete; the full engine, tracking, Claude Code adapter, six-command CLI, ten bundled rules, hermetic CI, and the README/worked-example/primitive-reference docs all ship.*
