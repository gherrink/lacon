...
- **T-04-09 (terminal injection via stored ANSI) mitigated as designed:** the raw column reproduces stored bytes verbatim (byte-fidelity is the Phase 6 SC3 contract); rows print as owned `String`s via normal `println!` (no raw-fd passthrough). The filtered column is the safe-to-read view (rules typically `strip_ansi`).
