...
- **User-authored pipes** (e.g. `cmd | grep foo`): the matched argv is wrapped as a unit. Whether the pipe is included in the wrapped argv depends on how the rule matcher resolves it — that detail belongs in `filter-rule-schema.md` and `specs/chained-commands.md` (added 2026-05-06), not here.
